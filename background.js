...
const tenSecondsCache = new (dist_default())(10 * 1000);
async function fetchUsingChatGptCookie(port, question, errorCallback) {
    const accessToken = await getAccessToken().catch((error) => errorCallback(JSON.parse(error.message)));
    if (!accessToken) {
        tenSecondsCache.delete(KEY_ACCESS_TOKEN);
        return;
    }
    let conversationId;
...
async function getAccessToken() {
    if (tenSecondsCache.get(KEY_ACCESS_TOKEN)) {
        return tenSecondsCache.get(KEY_ACCESS_TOKEN);
    }
    ...
    tenSecondsCache.set(KEY_ACCESS_TOKEN, data.accessToken);
    return data.accessToken;
}

;// CONCATENATED MODULE: ./src/background/chatapi.ts
...
async function logEvent(eventName, userId, loginType) {
    try {
...
;// CONCATENATED MODULE: ./src/background/crosssell.ts


const XSELL_CONFIG_URL = "https://xsell.api.hellogpt.chat";
let xSellConfig = null;
async function crosssell_getXSellBanner() {
    if (!xSellConfig) {
        xSellConfig = await getXSellBanner();
    }
    if (!xSellConfig || hasConfigExpired(xSellConfig)) {
        xSellConfig = await fetchXSellConfig();
        await setXSellBanner(xSellConfig);
    }
    if (isInvalidConfig(xSellConfig)) {
        return null;
    }
    return xSellConfig;
}
async function fetchXSellConfig() {
    return fetchConfig().then((response) => {
        if (response !== null) {
            return response;
        }
        return null;
...
async function fetchConfig() {
    return new Promise((resolve, reject) => {
        try {
            fetch(XSELL_CONFIG_URL, { method: "GET" })
                .then((response) => {
                if (response.ok) {
                    resolve(response.json());
                }
                else {
                    console.error("Non-ok status while fetching config:" + response);
                    resolve(null);
                }
            })
                .catch((err) => {
            ...
            console.error("Error occurred while fetching config" + exception);
            resolve(null);
        }
    });
}
function hasConfigExpired(config) {
    if (!config.expiryTimestampMs || config.expiryTimestampMs === 0) {
        return true;
    }
    return isAfter(new Date(), config.expiryTimestampMs);
}
function isInvalidConfig(config) {
    return (config == null ||
        !config.title ||
        config.title.length == 0 ||
        hasConfigExpired(config));
}
...
const oneHourCache = new (dist_default())(3600 * 1000);
...
async function getCachedLoginType() {
    let loginType = await oneHourCache.get(KEY_LOGIN_TYPE);
    if (loginType === undefined) {
        loginType = await getLoginType();
        await oneHourCache.set(KEY_LOGIN_TYPE, loginType);
    }
    return loginType;
}
...
async function getPromptWithContent(content, prompt) {
...
chrome.runtime.onConnect.addListener((port) => {
    port.onMessage.addListener(async (msg) => {
        ...
        const promptWithContent = await getPromptWithContent(msg.pageContent, msg.promptString);
        try {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === "ANALYTICS") {
        Promise.all([getCachedUserId(), getCachedLoginType()]).then(([userId, loginType]) => logEvent(message.data.event_name, userId, loginType));
    }
    else if (message.type === "PROMPT_OPTIONS") {
        getPromptList().then((prompts) => sendResponse({ prompts: prompts }));
    }
    else if (message.type === "GET_DEFAULT_PROMPT") {
        getDefaultPrompt().then((prompt) => sendResponse({ value: prompt.promptString }));
    }
    else if (message.type === "CHANGE_DEFAULT_PROMPT") {
        setDefaultPrompt(message.index).then(() => sendResponse({}));
    }
    else if (message.type === "GET_CROSS_SELL") {
        crosssell_getXSellBanner()
            .then((xSellValue) => sendResponse({ value: xSellValue }))
            .catch((_) => sendResponse({ value: null }));
    }
    return true;
});
chrome.action.onClicked.addListener(async (tab) => {
    ...
    logEvent("extension_triggered", await getCachedUserId(), await getCachedLoginType());
});
chrome.runtime.onInstalled.addListener(async (details) => {
    if (details.reason == chrome.runtime.OnInstalledReason.INSTALL) {
        logEvent("extension_installed", await getCachedUserId(), await getCachedLoginType());
    }
    else if (details.reason == chrome.runtime.OnInstalledReason.UPDATE) {
        chrome.runtime.openOptionsPage();
        await migrateLegacyPrompt();
    }
});
