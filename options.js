...
async function getLegacyPrompt() {
...
async function resetPrompt() {
    ...
    userSettings.prompts = null;
    await setUserSettings(userSettings);
    await chrome.storage.local.remove(PROMPT_KEY);
}
async function getPromptList() {
    const userSettings = await getUserSettings();
    if (userSettings.prompts && userSettings.prompts.length > 0) {
        return userSettings.prompts;
    }
    const defaultPromptList = [{ promptString: DEFAULT_PROMPT, isDefault: true }];
    await setPromptList(defaultPromptList);
...
async function migrateLegacyPrompt() {
    ...
    const legacyPrompt = await getLegacyPrompt();
    if (legacyPrompt) {
        await setPromptList([{ promptString: legacyPrompt, isDefault: true }]);
    }
}
...
async function setDefaultPrompt(index) {
    let prompts = await getPromptList();
    prompts = prompts.map((prompt) => {
        prompt.isDefault = false;
        return prompt;
    });
    prompts[index].isDefault = true;
    await setPromptList(prompts);
}
async function setPromptList(prompts) {
    let userSettings = await getUserSettings();
    userSettings.prompts = prompts;
    await setUserSettings(userSettings);
}
...
async function getXSellBanner() {
    let userSettings = await getUserSettings();
    return userSettings.xSellBanner;
}
async function setXSellBanner(value) {
    let userSettings = await getUserSettings();
    userSettings.xSellBanner = value;
    await setUserSettings(userSettings);
}
...
async function getShownArkosePopup() {
    let userSettings = await getUserSettings();
    return userSettings.shownArkoseFixPopupV2;
}
async function setArkoseFixPopup() {
    let userSettings = await getUserSettings();
    userSettings.shownArkoseFixPopupV2 = true;
    await setUserSettings(userSettings);
}
...
addEventListener("load", async () => onLoad());
async function onLoad() {
    await refreshPromptsFromDb();
    setupPromptSection();
    ...
    const shownFixPopup = await getShownArkosePopup();
    if (!shownFixPopup) {
        alert("ReaderGPT Announcement !!\n\nThe ChatGPT login is now working again.\n\nPlease feel free to switch the login method to ChatGPT to summarise your web pages for free without API key.");
        await setArkoseFixPopup();
    }
}
async function refreshPromptsFromDb() {
    const promptList = await getPromptList();
    setPromptsInUi(promptList);
}
function setPromptsInUi(prompts) {
    jquery_default()("#promptList").empty();
    prompts.forEach((prompt) => {
        const row = jquery_default()(createPromptRow(prompt));
        row.find("input").prop("checked", prompt.isDefault);
        jquery_default()("#promptList").append(row);
    });
}
function createPromptRow(prompt) {
    const trashIcon = chrome.runtime.getURL("images/trash.svg");
    return (`<li class='list-group-item d-flex justify-content-between align-items-center pt-0 pb-0'>` +
        `<input type='radio' name='promptRadio'>` +
        `<span class="m-2">${prompt.promptString}</span>` +
        `<button type="button" class="btn btn-link btn-xs pull-right deletePromptBtn ms-auto"><img src="${trashIcon}"/></button></li>`);
}
function setupPromptSection() {
    jquery_default()("#promptInput").on("input", () => {
        jquery_default()("#promptForm").removeClass("was-validated");
    });
    jquery_default()("#addPromptButton").on("click", async () => {
        var _a;
        let prompt = jquery_default()("#promptInput").val();
        if (!prompt) {
            return;
        }
        (_a = jquery_default()("#promptForm")) === null || _a === void 0 ? void 0 : _a.addClass("was-validated");
        if (!prompt.endsWith(":")) {
            prompt = prompt + ":";
        }
        let existingPrompts = await getPromptList();
        existingPrompts = [
            { promptString: prompt, isDefault: false },
            ...existingPrompts,
        ];
        // New prompt is chosen as default.
        selectAndSavePrompts(existingPrompts, 0);
    });
    jquery_default()(document).on("click", ".deletePromptBtn", async function () {
        const deleteItemIndex = jquery_default()(this).closest("li").index();
        const existingPrompts = await getPromptList();
        let defaultPromptIndex = existingPrompts.findIndex((prompt) => prompt.isDefault);
        if (existingPrompts.length > 1) {
            existingPrompts.splice(deleteItemIndex, 1);
        }
        if (defaultPromptIndex >= deleteItemIndex) {
            defaultPromptIndex = Math.max(0, defaultPromptIndex - 1);
        }
        selectAndSavePrompts(existingPrompts, defaultPromptIndex);
    });
    jquery_default()("#resetButton").on("click", async () => {
        await resetPrompt();
        await refreshPromptsFromDb();
    });
    jquery_default()(document).on("change", "input:radio[name=promptRadio]", async function () {
        const selectItemIndex = jquery_default()(this).closest("li").index();
        let existingPrompts = await getPromptList();
        selectAndSavePrompts(existingPrompts, selectItemIndex);
    });
}
async function selectAndSavePrompts(prompts, selectIndex) {
    prompts = prompts.map((prompt) => {
        prompt.isDefault = false;
        return prompt;
    });
    prompts[selectIndex].isDefault = true;
    setPromptsInUi(prompts);
    await setPromptList(prompts);
}
function selectLoginType(loginType) {
    var _a, _b, _c, _d;
    if (loginType == LoginType.API_KEY) {
        document.getElementById("openAiKeyRadio").checked =
            true;
        ...
        document.getElementById("chatGptLoginRadio").checked =
            true;
